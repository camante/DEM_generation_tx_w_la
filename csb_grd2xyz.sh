...
#!/bin/sh
function help () {
echo "grd2tif2chunks2xyz.sh - A script that unzips any .gz and converts all .grd files in a directory to .tif files and then chunks and then converts to xyz."
...
#get count of grd files
total_files=$(ls -1 | grep '\.grd$' | wc -l)
echo "Total number of grd files to process:" $total_files
...
do
	...
	then
		...
		echo -- Converting to negative depths
		gdal_calc.py -A $(basename $i .tif)"_resamp_tmp.tif" --outfile=$(basename $i .tif)"_resamp.tif" --calc="A*-1"
		rm $(basename $i .tif)"_resamp_tmp.tif"
		...
		gdal_translate -of Gtiff $i $(basename $i .grd)".tif"

		echo -- Converting to negative depths
		gdal_calc.py -A $(basename $i .grd)".tif" --outfile=$(basename $i .grd)"_tmp.tif" --calc="A*-1"
		mv $(basename $i .grd)".tif" tmp2.tif
		mv $(basename $i .grd)"_tmp.tif" $(basename $i .grd)".tif"
		rm tmp2.tif
		rm $(basename $i .grd)".tif.aux.xml"
		input_file=$(basename $i .grd)".tif"
		echo input_file is $input_file
	fi
	...
	while [ "$(bc <<< "$xoff < $x_dim")" == "1"  ]; do
	    ...
	    then
		    ...
		    rm $input_name"_chunk_"$chunk_name"_tmp2.tif.aux.xml"
	 	fi
